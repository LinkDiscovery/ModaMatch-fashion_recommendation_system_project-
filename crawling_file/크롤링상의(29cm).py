...
# 이미지 저장 디렉토리 설정
image_save_path = './data/images/29cm상의'
...
def make_df(image_links, product_names, product_prices, product_links, file_num):
    ...
    # 데이터프레임을 엑셀 파일로 저장
    file_path = os.path.join(save_path, f"상품정보상의(29cm)_{file_num}.xlsx")
    product_df.to_excel(file_path, index=False)
    print(f"{file_path} 파일이 저장되었습니다.")

# URL을 동적으로 생성하여 페이지를 순회하면서 크롤링 수행
base_url = 'https://shop.29cm.co.kr/category/list?colors=&categoryLargeCode=268100100&categoryMediumCode=268103100&categorySmallCode=&minPrice=&maxPrice=&isFreeShipping=&excludeSoldOut=&isDiscount=&brands=&sort=RECOMMEND&defaultSort=RECOMMEND&sortOrder=DESC&tag=&extraFacets=&page='
page_num = 1
