...
# 이미지 저장 디렉토리 설정
image_save_path = './data/images/베이지블랑상의3'
...
def make_df(image_links, product_names, product_prices, product_links):
    ...
    # 데이터프레임을 엑셀 파일로 저장
    file_path = os.path.join(save_path, "상품정보상의2(베이지블랑).xlsx")
    product_df.to_excel(file_path, index=False)
    print(f"{file_path} 파일이 저장되었습니다.")

# URL을 동적으로 생성하여 페이지를 순회하면서 크롤링 수행
base_url = 'https://smartstore.naver.com/beige-blanc/category/cdbe3d29847b40539cd6c24a4dd0292f?st=POPULAR&dt=IMAGE&page='
url_suffix = '&size=80'
page_num = 3
