...
# 이미지 저장 디렉토리 설정
image_save_path = './data/images/베이지블랑아우터2'
...
def make_df(image_links, product_names, product_prices, product_links):
    ...
    # 데이터프레임을 엑셀 파일로 저장
    file_path = os.path.join(save_path, "상품정보아우터2(베이지블랑).xlsx")
    product_df.to_excel(file_path, index=False)
    print(f"{file_path} 파일이 저장되었습니다.")

# URL을 동적으로 생성하여 페이지를 순회하면서 크롤링 수행
base_url = 'https://smartstore.naver.com/beige-blanc/category/3350090817a84814aa5bd80f2150f656?st=POPULAR&dt=IMAGE&page='
url_suffix = '&size=80'
page_num = 2
...
while page_num <= 2:
    url = base_url + str(page_num) + url_suffix
    driver.get(url)
    ...
    products = bs_obj.find_all('div', class_='_2kRKWS_t1E')
    ...
    for product in products:
        try:
            # 필요한 데이터를 추출
            product_link_tag = product.find('a', class_='_2id8yXpK_k linkAnchor _nlog_click _nlog_impression_element')
            if not product_link_tag:
                continue
            link_suffix = product_link_tag['href']
            full_link = 'https://smartstore.naver.com' + link_suffix
            
            # 이미 수집된 링크인지 확인
            if full_link in collected_links:
                continue
            
            img_tag = product.find('img', class_='_25CKxIKjAk')
            if img_tag:
            ...
            # 가격 추출
            product_price_tag = product.find('span', class_='_2DywKu0J_8')
            ...
            if full_link and product_name and product_price and image_url:
    ...
    # 총 수집된 데이터가 10000개가 넘으면 크롤링 종료
    if total_collected_count >= 10000:
        print("Limit reached. Stopping the crawl.")
        break
