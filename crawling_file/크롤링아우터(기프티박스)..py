...
# 이미지 저장 디렉토리 설정
image_save_path = './data/images/기프티박스아우터'
...
def make_df(image_links, product_names, product_prices, product_links):
    ...
    # 데이터프레임을 엑셀 파일로 저장
    file_path = os.path.join(save_path, f"상품정보아우터(기프티박스).xlsx")
    product_df.to_excel(file_path, index=False)
    print(f"{file_path} 파일이 저장되었습니다.")

# URL을 동적으로 생성하여 페이지를 순회하면서 크롤링 수행
base_url = 'https://gifteabox.com/product/list.html?cate_no=93&page='
page_num = 1
