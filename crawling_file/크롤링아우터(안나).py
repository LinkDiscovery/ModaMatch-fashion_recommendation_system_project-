...
# 이미지 저장 디렉토리 설정
image_save_path = './data/images/안나셔츠'
...
def make_df(image_links, product_names, product_prices, product_links):
    ...
    # 데이터프레임을 엑셀 파일로 저장
    file_path = os.path.join(save_path, "상품정보셔츠(안나).xlsx")
    product_df.to_excel(file_path, index=False)
    print(f"{file_path} 파일이 저장되었습니다.")

# URL을 동적으로 생성하여 페이지를 순회하면서 크롤링 수행
base_url = 'https://annanmode.com/product/list.html?cate_no=58/?page='
page_num = 1
...
while page_num <= 2:
    ...
    products = bs_obj.find_all('div', class_='-thumb')
    ...
    for product in products:
        try:
            # 필요한 데이터를 추출
            product_link_tag = product.find('a', {'name': lambda x: x and x.startswith('anchorBoxName_')})
            if not product_link_tag:
                continue
            link_suffix = product_link_tag['href']
            full_link = 'https://annanmode.com' + link_suffix
            ...
            # 가격 추출
            product_price_element = product.find_next('div', rel='판매가')
            if product_price_element:
                price_span = product_price_element.find_all('span', style='font-size:12px;color:#0b0b0b;')
                if len(price_span) > 1:  # 두 번째 span 태그가 있는지 확인
    ...
    # 수집된 데이터의 갯수가 증가하지 않거나 총 수집된 데이터가 10000개가 넘으면 크롤링 종료
    if total_collected_count == previous_total_collected_count or total_collected_count >= 10000:
        print("No more data collected or limit reached. Stopping the crawl.")
        break
