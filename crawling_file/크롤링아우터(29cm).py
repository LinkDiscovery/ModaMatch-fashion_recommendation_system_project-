...
# 이미지 저장 디렉토리 설정
image_save_path = './data/images/29cm아우터'
...
# 데이터를 저장하는 함수
def make_df(image_links, product_names, product_prices, product_links, file_num):
    ...
    # 데이터프레임을 엑셀 파일로 저장
    file_path = os.path.join(save_path, f"상품정보아우터(29cm)_{file_num}.xlsx")
    product_df.to_excel(file_path, index=False)
    print(f"{file_path} 파일이 저장되었습니다.")

# URL을 동적으로 생성하여 페이지를 순회하면서 크롤링 수행
base_url = 'https://shop.29cm.co.kr/category/list?colors=&categoryLargeCode=268100100&categoryMediumCode=268102100&categorySmallCode=&minPrice=&maxPrice=&isFreeShipping=&excludeSoldOut=&isDiscount=&brands=&sort=RECOMMEND&defaultSort=RECOMMEND&sortOrder=DESC&tag=&extraFacets=&page='
page_num = 1
total_collected_count = 0  # 전체 수집된 데이터의 갯수
collected_links = set()  # 수집된 링크를 저장하는 집합
previous_total_collected_count = 0  # 이전 페이지까지의 총 수집 데이터 갯수
...
while page_num <= 300:
    ...
    products = bs_obj.find_all('li', class_='css-1teigi4 e1114pfz0')
    ...
    for product in products:
        try:
            # 필요한 데이터를 추출
            product_link_tag = product.find('a', class_='css-5cm1aq e1114pfz1')
            if not product_link_tag:
                continue
            full_link = product_link_tag['href']
            ...
            # 가격 추출
            product_price_tag = product.find('strong', class_='css-s9qxfl e1114pfz8')
    ...
    # 중간 저장
    if total_collected_count >= file_num * 10000:
        make_df(image_links, product_names, product_prices, product_links, file_num)
        image_links = []
        product_names = []
        product_prices = []
        product_links = []
    ...
    if total_collected_count == previous_total_collected_count:
        print("No more data collected. Stopping the crawl.")
        break
    
    previous_total_collected_count = total_collected_count
