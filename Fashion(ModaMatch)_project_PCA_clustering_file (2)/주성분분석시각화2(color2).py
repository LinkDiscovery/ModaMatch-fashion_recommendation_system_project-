...
# 이미지 경로 설정
data_dir = 'data/9oz_sales_top100_image'
...
# 채널별로 PCA 적용
def apply_pca_to_color_images(images, n_components=50):
    reshaped_images = images.reshape(images.shape[0], -1, 3)
    pca_models = [PCA(n_components=n_components) for _ in range(3)]
    pca_results = [pca_models[i].fit_transform(reshaped_images[:, :, i]) for i in range(3)]
    return pca_models, pca_results

# PCA 분석 수행
pca_models, pca_results = apply_pca_to_color_images(images, n_components=50)
